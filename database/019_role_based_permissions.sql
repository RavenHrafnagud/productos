-- -------------------------------------------------------------------------
-- Permisos por rol en Supabase (RLS + grants):
-- Roles esperados: Administrador, Gerente, Usuario.
--
-- 1) Administrador: acceso total en todas las areas.
-- 2) Gerente: acceso total excepto area usuarios.
-- 3) Usuario: acceso solo al area ventas (con lectura de productos/sucursales).
-- -------------------------------------------------------------------------
begin;
...
-- Catalogo.productos: admin/gerente full + usuario lectura.
do $$
declare
  policy_row record;
begin
  if to_regclass('catalogo.productos') is not null then
    execute 'alter table catalogo.productos enable row level security';
    ...
    loop
      execute format('drop policy if exists %I on catalogo.productos', policy_row.policyname);
    end loop;

    execute $q$
      create policy productos_admin_manager_all on catalogo.productos
      for all to authenticated
      using (auth.uid() is not null and public.has_role(auth.uid(), array['administrador','admin','gerente']))
      with check (auth.uid() is not null and public.has_role(auth.uid(), array['administrador','admin','gerente']))
    $q$;
    ...
    execute $q$
      create policy productos_sales_read on catalogo.productos
      for select to authenticated
      using (auth.uid() is not null and public.has_role(auth.uid(), array['usuario']))
    $q$;

    grant select, insert, update, delete on catalogo.productos to authenticated;
  end if;
end $$;

-- Ventas: admin/gerente/usuario pueden operar ventas.
do $$
declare
  policy_row record;
begin
  if to_regclass('ventas.ventas') is not null then
    execute 'alter table ventas.ventas enable row level security';
    ...
    loop
      execute format('drop policy if exists %I on ventas.ventas', policy_row.policyname);
    end loop;

    execute $q$
      create policy ventas_roles_all on ventas.ventas
      for all to authenticated
      using (auth.uid() is not null and public.has_role(auth.uid(), array['administrador','admin','gerente','usuario']))
      with check (auth.uid() is not null and public.has_role(auth.uid(), array['administrador','admin','gerente','usuario']))
    $q$;

    grant select, insert, update, delete on ventas.ventas to authenticated;
  end if;
end $$;

-- Ajusta funcion legacy si existe.
do $$
begin
  if to_regclass('operaciones.has_local_access') is not null then
    execute $q$
      create or replace function operaciones.has_local_access(p_local_id uuid)
      returns boolean
      language sql
      security definer
      set search_path = public, identidad, operaciones
      as $fn$
        select public.has_role(auth.uid(), array['administrador','admin','gerente','usuario']);
      $fn$;
    $q$;

    execute 'grant execute on function operaciones.has_local_access(uuid) to authenticated';
  end if;
end $$;
